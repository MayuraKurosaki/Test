...
'    Frame1.Width = 225
    '-------------------
    MsgBox "IsWindowUnicodeUserForm:" & IsWindowUnicode(hWnd), vbOKOnly
    
    hWndFrame = Me.Frame1.[_GethWnd]
    
    Set LView = New ListView
    
'    If Not LView.Init(Frame1) Then Set LView = Nothing: Debug.Print "Init Error": Exit Sub
    If Not LView.Init(hWndFrame, 0, 0, Me.Frame1.Height, Me.Frame1.Width) Then Set LView = Nothing: Debug.Print "Init Error": Exit Sub
'    If Not LView.Init(hWnd, 0, 0, Me.Frame1.Height, Me.Frame1.Width) Then Set LView = Nothing: Debug.Print "Init Error": Exit Sub
'    If Not LView.Init(hWnd, 6, 6, 160, 300) Then Set LView = Nothing: Debug.Print "Init Error": Exit Sub
...
'    If Not LView.SetTextBkColor(RGB(0, 0, 255)) Then Set LView = Nothing: Exit Sub
    ...
    DBFileName = SelectFile
    LView.CsvToVListView DBFileName, , "Auto"
    
'    MeasureProcTime 0
    LView.SetAllColumnWidthAuto
'    Debug.Print "SetAllColumnWidthAuto:" & MeasureProcTime(1)
End Sub

'Private Sub LView_ItemClick(ByVal ListItem As ListItem)
'    TextBox1.Text = ListItem.Index
''    TextBox2.Text = LView.LabelText(iItem, iSubItem)
'    Dim iItem As Long
...
Private Sub LView_ItemClick(ByVal iItem As Long, ByVal iSubItem As Long)
    TextBox1.Text = iItem
'    TextBox2.Text = LView.LabelText(iItem, iSubItem)
    TextBox2.Text = LView.GetItemText(iItem, iSubItem)
...
'    test_DialogFont Frame1
    LView.Font.test_DialogFont
