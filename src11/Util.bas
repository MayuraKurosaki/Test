...
Enum ErrCode
    ...
    WrongType = 520
    '
    UnsupportedFormat = 525
    CouldNotDetectFormat = 526
    NodeIsNotObject = 527
    CannotAddNodeAsField = 528
    YAMLParseError = 529
    XMLParseError = 530
    JSONParseError = 531
    ...
    ScriptRunTime = 550
    VarNotExists = 551
    NumCannotConvert = 552
    ScriptFileNotExists = 553
    ScriptSyntax = 554
    ScriptParameterError = 555
    LogWriteError = 556
End Enum
...
' 配列の次元数を返す(配列以外の場合は0を返す)
Public Function ArrayDemension(ByRef aArray() As Variant) As Long
    ArrayDemension = 0
    
    If VBA.IsArray(aArray) Then
        Dim tmp As Variant
        Dim i As Long
...
' 配列の要素数を返す(配列以外の場合は0を返す)
Public Function ArrayCount(ByRef aArray() As Variant, Optional ByVal aDimension As Long = 1) As Long
    If VBA.IsArray(aArray) Then
        ArrayCount = UBound(aArray) - LBound(aArray) + 1
    Else
        ArrayCount = 0
    End If
...
Public Function MeasureProcTime(ByRef Mode As Long) As Double
'    Static OnMeasure As Boolean
    ...
    Select Case Mode
        ...
        Case 1: 'Stop
            MeasureProcTime = (timeGetTime - StartTime) / 1000
    End Select
'    If OnMeasure Then
'        Debug.Print "MeasureProcTime End"
'        MeasureProcTime = (timeGetTime - StartTime) / 1000
'        StartTime = 0
'        OnMeasure = False
'    Else
'        Debug.Print "MeasureProcTime Start"
'        MeasureProcTime = 0
'        StartTime = timeGetTime
'        OnMeasure = True
'    End If
...
''クラス疑似コンストラクタ呼び出し用
'Public Function CreateInstance(ByRef pObj As Initializable, ParamArray p() As Variant) As Object
'    Dim p2() As Variant
'
'    If UBound(p) <> -1 Then
'        ReDim p2(UBound(p))
'    Else
'        Set CreateInstance = pObj.CreateInstance(p2)
'        Exit Function
'    End If
'
''    If UBound(p) = -1 Then
